# -*- coding: utf-8 -*-
"""
Created on Sat Aug 22 18:31 2026

RxSM - Real Singlet Extension of the Standard Model, 
Considerations: no Z_2 symmetry, a Tadpole Term, and no singlet vev.

This file is a copy of xSM_Tadpole.py but with the bosonic masses fixed.
...
class RxSM(generic_potential.generic_potential):
    ...
    def boson_massSq(self, X, T, Goldstone=True):
        ...
        idx  particle          dof    c
         ...
         3   W transverse       4    1/2   (2 charged * 2 transverse)
         4   Z transverse       2    1/2
         5   W longitudinal     2    3/2   (2 charged)
         6   Z longitudinal     1    3/2
         7  ph longitudinal     1    3/2
         """
        X = np.asarray(X, dtype=float)
        h = X[..., 0]
        s = X[..., 1]
        ...
        aZ = 0.25*self.g2**2*h**2 + PiWL
        dZ = 0.25*self.g1**2*h**2 + PiBL
        bZ = 0.25*self.g1*self.g2*h**2
        discZ = np.sqrt((0.5*(aZ-dZ))**2 + bZ**2)

        mZLsq = 0.5*(aZ+dZ) + discZ
        mgLsq = 0.5*(aZ+dZ) - discZ

        # --- Cosmo Transitions Output ---
        masses = np.stack([m1sq, m2sq, mGsq_T,
                           mWsq,  mZsq,
                           mWLsq, mZLsq, mgLsq], axis=-1)
        dofs   = np.array([1.,  1.,  3.,
                           4.,  2.,
                           2.,  1., 1.])
